...
//user should give their own values here
...
//set accel_on_delay to 3
  ...
  if (i2c_smbus_write_byte_data (mpu_client, MOT_DETECT_CTRL, write_byte) < 0)
    {
      printk (KERN_ALERT "Cannot write byte to device");
      return -1;
    }
...
MODULE_AUTHOR ("gt");
MODULE_DESCRIPTION ("simply enable motion interrupts on i2c");
